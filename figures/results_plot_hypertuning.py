...
#	r = np.arange(2)
	nb_xps = len(p)
	nb_clusters = len(p[list(p.keys())[0]]['mean'])
	cluster_width = barWidth * (nb_xps * (1 + bar_gap_perc) - bar_gap_perc)
	r = [(cluster_gap + cluster_width) * i for i in range(nb_clusters)]
# 	r = [0, gap + barWidth * 3]
#	r = [0 - barWidth, nb_xp * barWidth + gap * 0.5 - barWidth]
	...
	for i, xp in enumerate(p.keys()):
		...
		y_err = p[xp]['interval']
		# Create blue bars
		r_cur = [x + barWidth * (i - 1) * (1 + bar_gap_perc) - cluster_width / 2 for x in r]
		plt.bar(r_cur,
			bars, width=barWidth, color=palette(i),
			edgecolor='black', linewidth=0.2,
			yerr=y_err, error_kw=dict(lw=0.5, capsize=3, capthick=0.5),
			label=xp)
		# Add values upon the bars.
		for index, value in enumerate(r_cur):
			plt.text(value - 0.045, (bars[index] * 1.03 if bars[index] > 0 else 0),
			'{:.1f}'.format(bars[index]), color=palette(i), size=3)

# 		MAX_Y = np.max(bars + [MAX_Y])

	# general layout
	ax.set_xticks(r)
	ax.set_xticklabels(['train', 'valid', 'test'], ha='right') # ['train errors', 'test errors'])
...
def mean_minmax_interval(data):
	m = np.mean(data)
	h = np.max(data) - np.mean(data)
	return m, h
...
def compute_perf(results, app_or_test, intv_type='confidence'):
	if intv_type == 'confidence':
		return mean_confidence_interval(results[app_or_test])
	elif intv_type == 'minmax':
		return mean_minmax_interval(results[app_or_test])
	else:
		raise ValueError('Interval type "%s" can not be recognized, possible '
				   'candidates include "confidence" and "minmax".' % intv_type)
...
def compute_displayable_results(results_by_xp):
	...
	for xp in results_by_xp.keys():
		p[xp] = {}
		p[xp]["mean"] = [0] * len(results_by_xp[xp])
		p[xp]["interval"] = [0] * len(results_by_xp[xp])
		for i, (k, v) in enumerate(results_by_xp[xp].items()):
			p[xp]["mean"][i], p[xp]["interval"][i] = compute_perf(results_by_xp[xp], k,
														 intv_type='confidence')
...
# 	DS_Name_List = ['poly200', 'thermo_exp', 'thermo_cal']
	...
	Activation_Fn_List = ['relu', 'elu', 'leaky_relu', 'selu', 'gelu', 'linear',
					'tanh', 'softmax', 'sigmoid', 'exponetial'][0:-1]
...
# 	xps = ["random", "expert", "fitted"]
	results_by_xp = organize_results_by_xps(results, Activation_Fn_List)
...
#	plt.rc('figure', titlesize=15)  # fontsize of the figure title
	...
	#ax3 = plt.subplot(223)
	#ax4 = plt.subplot(224)
	fig = plt.figure(figsize=(5 * nb_cols, 2.12 * nb_rows + 0.8))
