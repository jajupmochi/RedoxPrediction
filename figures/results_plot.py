...
#	r = [0 - barWidth, nb_xp * barWidth + gap * 0.5 - barWidth]
	...
	for i, xp in enumerate(p.keys()):
		...
		for index, value in enumerate(r_cur):
			...
			'{:.1f}'.format(bars[index]), color=palette(i), size=6)
...
# 	str_stra = ('.stratified' if stratified else '')
	...
	for ds_name in DS_Name_List:
		...
		fn = root_dir + 'results.' + '.'.join([v for k, v in task.items()]) + '.shuffle.5folds.no_seed.pkl'
...
#	plt.rc('figure', titlesize=15)  # fontsize of the figure title
	...
	#ax3 = plt.subplot(223)
	#ax4 = plt.subplot(224)
	fig = plt.figure(figsize=(3 * nb_cols, 2.12 * nb_rows + 0.8))
...
# 		 row_grid_list.append({'edit_cost': i, 'distance': j})
	...
	for fig_name in fig_name_list:
		...
		fn_prefix = 'results.' + fig_name + nm_stra
