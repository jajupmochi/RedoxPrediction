...
from redox_prediction.models.nn.utils import format_targets
...
def evaluate_epoch_mtgnn(
	...
	"""
	ave_meters = {key: AverageMeter(keep_all=False) for key in [
		'loss', 'loss_m', 'loss_p', 'loss_p1', 'loss_p2',
		'batch_time', 'data_time', 'metric_time'
	]}

	model.eval()

	with torch.no_grad():
		for i, (data1, data2, metric_score) in enumerate(val_loader):
			start_time = time.time()

			# Prepare input data:
			data1 = data1.to(device)
			data2 = data2.to(device)
			metric_score = metric_score.to(device)
			target1 = data1.y
			target2 = data2.y
			if criterion.__class__.__name__ == 'MetricTargetLoss':
				crit_name1 = criterion.func1.__class__.__name__
				crit_name2 = criterion.func2.__class__.__name__
			else:
				raise ValueError('criterion must be MetricTargetLoss for now.')
			target1 = format_targets(target1, crit_name2)
			target2 = format_targets(target2, crit_name2)

			# Log data time:
			ave_meters['data_time'].update(
				(time.time() - start_time) / target1.size(0), target1.size(0)
			)

			start_time = time.time()

			# Compute features:
			output1, embedding1 = model(data1, output='both')
			output2, embedding2 = model(data2, output='both')
			...
			# Compute loss:
			loss, loss_m, loss_p1, loss_p2 = criterion(
				embedding1, embedding2, metric_score,
				output1, target1, output2, target2
			)

			# Update average meters:
			ave_meters['metric_time'].update(
				kwargs['metric_pairwise_run_time'], target1.size(0)
			)
			ave_meters['loss'].update(loss.item(), target1.size(0))
			ave_meters['loss_m'].update(loss_m.item(), target1.size(0))
			ave_meters['loss_p'].update(
				loss_p1.item() + loss_p2.item(), target1.size(0)
				)
			ave_meters['loss_p1'].update(loss_p1.item(), target1.size(0))
			ave_meters['loss_p2'].update(loss_p2.item(), target1.size(0))
			...
			if verbose and (i + 1) % print_interval == 0:
				print(
					...
					f'Metric Time {ave_meters["metric_time"].val:.3f} '
					f'({ave_meters["metric_time"].avg:.3f})\t'
					f'Loss {ave_meters["loss"].val:.4f} ({ave_meters["loss"].avg:.4f})'
					f'Metric Loss {ave_meters["loss_m"].val:.4f} '
					f'({ave_meters["loss_m"].avg:.4f})\t'
					f'Predictor Loss {ave_meters["loss_p"].val:.4f} '
					f'({ave_meters["loss_p"].avg:.4f})\t'
					f'Predictor Loss 1 {ave_meters["loss_p1"].val:.4f} '
					f'({ave_meters["loss_p1"].avg:.4f})\t'
					f'Predictor Loss 2 {ave_meters["loss_p2"].val:.4f} '
					f'({ave_meters["loss_p2"].avg:.4f})\t'
				)

	return ave_meters['loss'].avg, ave_meters


def train_epoch_mtgnn(
	...
	ave_meters = {key: AverageMeter(keep_all=False) for key in [
		'loss', 'loss_m', 'loss_p', 'loss_p1', 'loss_p2',
		'batch_time', 'data_time', 'metric_time', 'alpha'
	]}

	model.train()

	for i, (data1, data2, metric_score) in enumerate(train_loader):
		start_time = time.time()
		...
		# Log batch time and metric time:
		ave_meters['batch_time'].update(
			(time.time() - start_time) / target1.size(0), target1.size(0)
		)
		ave_meters['metric_time'].update(
		...
		ave_meters['loss'].update(loss.item(), target1.size(0))
		ave_meters['loss_m'].update(loss_m.item(), target1.size(0))
		ave_meters['loss_p'].update(loss_p1.item() + loss_p2.item(), target1.size(0))
		ave_meters['loss_p1'].update(loss_p1.item(), target1.size(0))
		ave_meters['loss_p2'].update(loss_p2.item(), target1.size(0))
	...
	# Log alpha:
	ave_meters['alpha'].update(criterion.alpha, 1)
...
def fit_model_mtgnn(
	...
	# Initialize the loss function. 'infer' and 'predictor_clf_activation' are
	# required in 'kwargs':
	...
	if loss_func.__class__.__name__ == 'MetricTargetLoss':
		optim_params = itertools.chain(
			model.parameters(), loss_func.parameters()
		)
	else:
	...
	if read_resu_from_file >= 2 and load_state_dict_from is not None and os.path.exists(
		...
		history = {key: AverageMeter(keep_all=ka) for key, ka in zip([
			'train_loss', 'train_loss_m', 'train_loss_p', 'train_loss_p1', 'train_loss_p2',
			'valid_loss', 'valid_loss_m', 'valid_loss_p', 'valid_loss_p1', 'valid_loss_p2',
			'valid_metric',
			'alpha',
			'epoch_time_fit',
			'batch_time_train', 'data_time_train', 'metric_time_train',
			'batch_time_valid', 'data_time_valid', 'metric_time_valid',
	...
	for epoch in range(start_epoch, max_epochs + 1):
		...
		# Train the model
		train_loss, train_meters = train_epoch_mtgnn(
			train_loader,
			model,
			optimizer,
			loss_func,
		...
		if if_valid_epoch(valid_loader, epoch, epochs_per_eval):
			valid_loss, valid_meters = evaluate_epoch_mtgnn(
				valid_loader,
				model,
				loss_func,
				device,
			...
			perf_valid, y_pred_valid, y_true_valid, predict_history = predict_mtgnn(
				valid_loader_task,
				model,
				metric,
				device,
		...
		history['train_loss_m'].update(train_meters['loss_m'].avg)
		history['train_loss_p'].update(train_meters['loss_p'].avg)
		history['train_loss_p1'].update(train_meters['loss_p1'].avg)
		history['train_loss_p2'].update(train_meters['loss_p2'].avg)
		# history['train_metric'].append(train_metric)
		if valid_loader is not None:
			history['valid_loss'].update(valid_loss)
			history['valid_loss_m'].update(valid_meters['loss_m'].avg)
			history['valid_loss_p'].update(valid_meters['loss_p'].avg)
			history['valid_loss_p1'].update(valid_meters['loss_p1'].avg)
			history['valid_loss_p2'].update(valid_meters['loss_p2'].avg)
		...
		history['alpha'].update(loss_func.alpha)
		...
		if verbose and (epoch < 10 or epoch % print_interval == 0):
			if if_valid_epoch(valid_loader, epoch, epochs_per_eval):
				print(
					...
					'LR: {:.5f} \t '
					'Alpha: {:.4f} \t '
					'(Train Loss M {:.4} \t P {:.4} \t P1 {:.4} \t P2 {:.4} \t '
					'Valid Loss M {:.4} \t P {:.4} \t P1 {:.4} \t P2 {:.4})'.format(
						epoch,
						train_loss,
						valid_loss,
						perf_valid,
						...
						loss_func.alpha,
						train_meters['loss_m'].avg,
						train_meters['loss_p'].avg,
						train_meters['loss_p1'].avg,
						train_meters['loss_p2'].avg,
						valid_meters['loss_m'].avg,
						valid_meters['loss_p'].avg,
						valid_meters['loss_p1'].avg,
						valid_meters['loss_p2'].avg,
					)
				)
			else:
				print(
					...
					'LR: {:.5f} \t '
					'Alpha: {:.4f} \t '
					'(Train Loss M {:.4} \t P {:.4} \t P1 {:.4} \t P2 {:.4})'.format(
						epoch,
						train_loss,
						epoch_time,
						optimizer.param_groups[0]['lr'],
