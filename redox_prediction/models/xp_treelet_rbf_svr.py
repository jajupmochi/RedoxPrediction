...
# -*- coding: utf-8 -*-
"""
Created on Wed May  4 16:20:00 2022
...
path_kw = '/treelet_rbf_svr/atom_bond_types/'
# path_kw = '/treelet_rbf_svr/1hot/'
...
# 		}))
	# for no scaling:
	hparams = list(ParameterGrid({
		'gamma': [5e-5, 1e-4, 5e-4, 0.001, 0.005, 0.01, 0.05, 0.1, 0.5],
		'C': [0.005, 0.01, 0.05, 0.1, 0.5, 1, 5],
	...
	for params in get_iters(hparams, desc='Hyperparam tuning'):
		...
		graph_kernel = Treelet(node_labels=nl_names, edge_labels=el_names,
						   ...
						   sub_kernel=functools.partial(gaussian_kernel, gamma=params['gamma']),
						   normalize=True,
						   verbose=0)
