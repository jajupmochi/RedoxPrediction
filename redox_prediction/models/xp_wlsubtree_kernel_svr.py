...
# -*- coding: utf-8 -*-
"""
Created on Tue May  3 16:39:36 2022
...
# path_kw = '/wlsubtree_kernel_svr/atom_bond_types/std_y/'
path_kw = '/wlsubtree_kernel_svr/1hot/std_y/'
...
def evaluate_model(X_train, y_train, X_valid, y_valid, X_test, y_test,
	...
	hparams = list(ParameterGrid({
		'height': [0, 1, 2, 3],
		'C': [0.005, 0.01, 0.05, 0.1, 0.5, 1, 5, 10, 20],
		'epsilon': [0.1, 0.5, 1, 5, 10],
		}))
# 	# for no scaling:
# 	hparams = list(ParameterGrid({
# 		'height': [0, 1, 2],
# 		'alpha': [0.001 * i for i in range(1, 10, 2)] + [0.01 * i for i in range(1, 10, 2)] + [0.1 * i for i in range(1, 10, 2)] + [1 * i for i in range(1, 10, 2)] + [10 * i for i in range(1, 4, 1)]
# 		}))
	...
	for params in get_iters(hparams, desc='Hyperparam tuning'):
		...
		graph_kernel = WLSubtree(node_labels=nl_names, edge_labels=el_names,
