...
# -*- coding: utf-8 -*-
"""
Created on Wed Jun  8 15:48:10 2022
...
from gcn_model import GCNModel, GCNDataset
...
# 	from sklearn.model_selection import ParameterGrid
	if use_best_params:
		params_grid = {
			'hidden_feats': [64], # a little influential, [1]
			'message_steps': [1],
			'weight': [True], # False
			'norm': ['both'], #
			'predictor_hidden_feats': [1024], # a little influence, [512]
			'batch_size': [8], # a liitle influential, [8]
			'learning_rate': [0.001], # [5e-4, 0.001, 1e-4] influential?, [5e-4]
	# 		'feat_drop': [0, 0.3, 0.6],
			'redurce_lr_factor': [0.5], # a liile influential, [0.5]
	# 		'residual': [False, True],
			'agg_activation': ['relu'],
			'bias': [True],
			'readout': ['mean'],
	...
	for idx_params, param_name in get_iters(enumerate(cur_params), desc='Hyperparam tuning', length=len(cur_params)): # for each hyper-parameter:
		...
		for param_val in params_grid[param_name][start_idx:]:
			...
			# Transform datasets.
			train_dataset = GCNDataset(X_train, y_train, batch_size=batch_size, shuffle=False)
			valid_dataset = GCNDataset(X_valid, y_valid, batch_size=batch_size, shuffle=False)
			test_dataset = GCNDataset(X_test, y_test, batch_size=batch_size, shuffle=False)
