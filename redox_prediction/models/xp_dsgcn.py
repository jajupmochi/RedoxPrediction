...
# -*- coding: utf-8 -*-
"""
Created on Tue Jun 14 15:16:36 2022
...
from dsgcn_model import DSGCNModel, DSGCNDataset
...
# 	train_dataset = DSGCNDataset(X_train, y_train, batch_size=4, shuffle=False) # @todo:
...
# 	from sklearn.model_selection import ParameterGrid
	if use_best_params:
		params_grid = { # @todo
			'hidden_feats': [256], # [256, 32, 64, 128], # influential?, [32]
			'message_steps': [2], # [2, 1, 3, 4], # a little influential, [1]
			'predictor_hidden_feats': [128], # [128, 256, 512, 1024], # a little influence, [512]
			'batch_size': [4], # [4, 8, 16], # a liitle influential, [8]
			'learning_rate': [0.0005], # [1e-4, 5e-4, 0.001], # [5e-4, 0.001, 1e-4] influential?, [5e-4]
			'feat_drop': [0.1], # [0.1, 0., 0.3],
			'kernel_drop': [0.0], # [0.1, 0., 0.3],
			'redurce_lr_factor': [0.5], # [0.5, 1, 0.2, 0.1], # a liile influential, [0.5]
			'residual': [False], # [False, True],
			'agg_activation': ['relu'],
			'bias': [False], # [False, True],
			'readout': ['meanmax'], # ['meanmax', 'max', 'mean'],
			'is_depthwise': [False],
			'predictor_feat_drop': [0.3], # [0.1, 0., 0.3],
			'predictor_bias': [False], # [True, False],
			'predictor_weight_decay': [1e-4], # [1e-4, 0.],
	...
	# 		'nb_kernels': [4], # @todo
		...
		params_grid = { # @todo
			'hidden_feats': [256, 32, 64, 128], # influential?, [32]
			'message_steps': [2, 1, 3, 4], # a little influential, [1]
			'predictor_hidden_feats': [128, 256, 512, 1024], # a little influence, [512]
			'batch_size': [4, 8, 16], # a liitle influential, [8]
			'learning_rate': [1e-4, 5e-4, 0.001], # [5e-4, 0.001, 1e-4] influential?, [5e-4]
			'feat_drop': [0.1, 0., 0.3],
			'kernel_drop': [0.1, 0., 0.3],
			'redurce_lr_factor': [0.5, 1, 0.2, 0.1], # a liile influential, [0.5]
			'residual': [False, True],
			'agg_activation': ['relu'],
			'bias': [False, True],
			'readout': ['meanmax', 'max', 'mean'],
			'is_depthwise': [False],
			'predictor_feat_drop': [0.1, 0., 0.3],
			'predictor_bias': [True, False],
			'predictor_weight_decay': [1e-4, 0.],
			'predictor_activation': ['relu'],
			'weight_decay': [1e-4, 0.],
	# 		'nb_kernels': [4], # @todo
			}
	...
	for idx_params, param_name in get_iters(enumerate(cur_params), desc='Hyperparam tuning', length=len(cur_params)): # for each hyper-parameter:
		...
		for param_val in params_grid[param_name][start_idx:]:
			...
			# Transform datasets.
			train_dataset = DSGCNDataset(X_train, y_train, batch_size=batch_size, shuffle=False)
			valid_dataset = DSGCNDataset(X_valid, y_valid, batch_size=batch_size, shuffle=False)
			test_dataset = DSGCNDataset(X_test, y_test, batch_size=batch_size, shuffle=False)
			...
			model = DSGCNModel(
				...
				is_depthwise=params['is_depthwise'],
				feat_drop=params['feat_drop'],
				kernel_drop=params['kernel_drop'],
				bias=params['bias'],
