...
class Featurizer:
	...
	def encode(self, inputs, **kwargs):
		...
		for name_feature, allowable_set in self.allowable_sets.items():
			feature = getattr(self, name_feature)(
				inputs, allowable_set=allowable_set
			)
			output += feature
...
class AtomFeaturizer(Featurizer):
	...
	def atom_type(
		...
		return one_hot_encode(
			atom.GetSymbol(), allowable_set, include_unknown_set
		)
	...
	def hybridization(
			self, atom, allowable_set=None, include_unknown_set=False,
			return_dim=False
	):
	...
	def n_valence(
		...
		if return_dim:
			return len(allowable_set) + (1 if include_unknown_set else 0)
		return one_hot_encode(
			atom.GetTotalValence(), allowable_set, include_unknown_set
		)
...
class BondFeaturizer(Featurizer):
	...
	def encode(self, bond):
		if bond is None:
			if not self.add_self_loop:
				raise Exception(
					'add_self_loop is set to False; input bond can not be None.'
				)
	...
	def bond_type(
			self, bond, allowable_set=None, include_unknown_set=False,
			return_dim=False
	):
	...
	def stereo(
			self, bond, allowable_set=None, include_unknown_set=True,
			return_dim=False
	):
...
class MolGNNFeaturizer(object):
	...
	def featurize(
		...
		for i_mol, smiles in zip(mol_indices, smiles_list):
			if self.add_3d_coords and self.use_atomic_charges != 'none':
				atom_features, atom_coords, atomic_charges, bond_features, pair_indices = self._featurize(
					smiles, i_mol=i_mol + 1
				)
				atom_coords_list.append(atom_coords)
				atomic_charges_list.append(atomic_charges)
			elif self.add_3d_coords:
				atom_features, atom_coords, bond_features, pair_indices = self._featurize(
					smiles, i_mol=i_mol + 1
				)
				atom_coords_list.append(atom_coords)
			elif self.use_atomic_charges != 'none':
				atom_features, atomic_charges, bond_features, pair_indices = self._featurize(
					smiles, i_mol=i_mol + 1
				)
				atomic_charges_list.append(atomic_charges)
			else:
				atom_features, bond_features, pair_indices = self._featurize(
					smiles
				)
		...
		if self.use_atomic_charges != 'none':
			...
			if self.atom_char_settings['scaling'] == 'auto':
				# in "auto" model_type, the scaler will be set to `MinMaxScaler()`
				...
				raise ValueError(
					'Atomic charges scaling model_type: "%s" can not be recognized. '
		...
		if self.return_format == 'tf':
			...
			raise ValueError(
				'I do not understand the return_format you give me. :('
			)
	...
	def _featurize(self, smiles: str, **kwargs) -> tuple:
		...
		if self.add_3d_coords or self.use_atomic_charges != 'none':
			coords, sdf_str = get_atoms_3d_coordinates(
				smiles, **self.coords_settings, **kwargs
			)
			molecule = sdf_to_rdkitmol(sdf_str, add_Hs=True, assign_stereo=True)
			if self.use_atomic_charges != 'none':
				atomic_charges = get_atomic_charges(
					# @todo: make sure it is compatible with the 3d coords return model_type.
		...
		atom_features, bond_features, pair_indices = self.graph_from_molecule(
			molecule
		)
	...
	def graph_from_molecule(self, molecule):
		...
		for atom in molecule.GetAtoms():
			atom_features.append(
				self.atom_featurizer.encode(atom, h_bond_infos=h_bond_infos)
			)
		...
		return np.array(atom_features), np.array(bond_features), np.array(
			pair_indices
		)
