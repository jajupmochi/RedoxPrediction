...
def print_latex_results(results, mode, rm_valid=False):
	print('\n----- Final performance in LaTeX format: -----\n')
	final_perf = {}
	# accuracy:
	for key in ['perf_train', 'perf_valid', 'perf_test']:
		ave, std = results[key]
		if mode == 'classif':
			# Multiple 100 for percentage:
			ave, std = ave * 100, std * 100
		str_latex = str(round(ave, 1)) + '$\pm$' + str(round(std, 1))
		print(key + ': ' + str_latex)
	...
	for key in ['history_train', 'history_valid', 'history_test', 'best_history']:
		...
		for key2 in [
			...
			if key2 in results[key]:
				ave, std = results[key][key2]
				# If the value < 0.01, then print it in scientific notation:
				if ave < 0.01:
					ave, std = '%.2e' % ave, '%.2e' % std
				else:
					ave, std = str(round(ave, 3)), str(round(std, 3))
				str_latex = ave + '$\pm$' + std
				print('  ' + key2 + ': ' + str_latex)
				final_perf[key][key2] = str_latex
