...
# path_kw = '/ged_bp_random_knn/atom_bond_types/'
path_kw = '/ged_bp_fitted_knn/atom_bond_types/'
# path_kw = '/ged_ipfp_random_knn/atom_bond_types/'
# path_kw = '/ged_ipfp_fitted_knn/atom_bond_types/'
# path_kw = '/ged_bp_random_knn/1hot/'
...
def evaluate_model(X_train, y_train, X_valid, y_valid, X_test, y_test,
	...
	for params in get_iters(hparams, desc='Hyperparam tuning'):
		...
		ged_model = GEDModel(
			  ed_method=('BIPARTITE' if '_bp_' in path_kw else 'IPFP'),
			  edit_cost_fun='CONSTANT',
			  init_edit_cost_constants=[3, 3, 1, 3, 3, 1],
			  optim_method=('random' if 'random' in path_kw else 'fitted'),
...
#			  normalize=True,
			  ...
			  verbose=(0 if '_bp_' in path_kw else 2)
			  )
		model = KNeighborsRegressor(metric='precomputed',
